...
class Player:
    ...
    def decide(self, game):
        ...
        while action != "shoot":
            ...
            if action == "use_item":
                self.strategy.decide_item(game, self)
            elif action == "look":
                game.print_items()
...
class HumanStrategy(Strategy):
    ...
    def decide_item(self, game, player):
        ...
        used_item = player.inventory.use_item(item_count, game, player)
        if not used_item:
...
class IaStrategy(Strategy):
    ...
    def decide_other_player(self, player):
        player_numbers = [1, 2]
        other_players = list(filter(player_numbers.remove, player.number))
